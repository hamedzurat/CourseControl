<script lang="ts">
  ...
  type Relation = {
    subjects: Array<{ id: number; code: string; name: string; type?: string; credits?: number; sections: any[] }>;
  };
  ...
  function doCreate() {
    if (!createSubjectId || groupForSubject(createSubjectId)) return;
  ...
  onMount(() => {
    ...
    loadMe().catch(() => {});
    apiFetch<Relation>('/relation.json')
      .then((r) => (rel = r))
      .catch(() => {});
    return () => unMe();
  });
...
<div class="mx-auto w-full max-w-5xl space-y-6 px-4 py-6">
  ...
  <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
    <Card>
      <CardHeader
        ><CardTitle>Create a group</CardTitle><CardDescription>Only for subjects you're enrolled in.</CardDescription
        ></CardHeader
      >
      <CardContent class="space-y-3">
        ...
        >
          <Select.Trigger class="w-full">{createTriggerContent}</Select.Trigger>
          <Select.Content
            ><Select.Group
              ><Select.Label>Enrolled subjects</Select.Label>{#each createOptions as opt (opt.value)}<Select.Item
                  value={opt.value}
                  label={opt.label}
                  disabled={!!groupForSubject(Number(opt.value))}>{opt.label}</Select.Item
                >{/each}</Select.Group
            ></Select.Content
          >
        </Select.Root>
        <Button class="w-full" onclick={doCreate} disabled={!createSubjectId || !!groupForSubject(createSubjectId)}
          >Create group</Button
        >
        {#if createSubjectId && groupForSubject(createSubjectId)}<p class="text-xs text-muted-foreground">
            You already have a group for this subject.
          </p>{/if}
      </CardContent>
    </Card>
    ...
    <Card>
      <CardHeader
        ><CardTitle>Join a group</CardTitle><CardDescription
          >Paste an invite code from the group leader.</CardDescription
        ></CardHeader
      >
      <CardContent class="space-y-3">
  ...
  <Card>
    <CardHeader
      ><CardTitle>My groups</CardTitle><CardDescription>Manage groups you're a member of.</CardDescription></CardHeader
    >
          ...
          {#each groups as g (g.groupId)}
            <div class="space-y-3 rounded-lg border p-4">
              <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div class="space-y-1">
                  <div class="font-medium">{subjectLabel(Number(g.subjectId))}</div>
                  <div class="text-xs text-muted-foreground">
                    Group #{g.groupId}{#if g.isLocked}
                      • locked{/if}{#if g.leaderUserId}
                      • leader: {g.leaderUserId}{/if}
                  </div>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                  {#if isLeader(g)}<Badge>Leader</Badge>{:else}<Badge variant="secondary">Member</Badge
                    >{/if}{#if g.isLocked}<Badge variant="secondary">Locked</Badge>{/if}
                </div>
              </div>
              <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                {#if isLeader(g)}
                  <div class="flex flex-col gap-2 md:flex-row md:items-center">
                    <Select.Root type="single" name="inviteCount" bind:value={inviteCount}
                      ><Select.Trigger class="w-[140px]">Invites: {inviteCount}</Select.Trigger><Select.Content
                        ><Select.Group
                          ><Select.Label>Count</Select.Label><Select.Item value="1" label="1">1</Select.Item
                          ><Select.Item value="2" label="2">2</Select.Item><Select.Item value="3" label="3"
                            >3</Select.Item
                          ><Select.Item value="5" label="5">5</Select.Item></Select.Group
                        ></Select.Content
                      ></Select.Root
                    >
              ...
              {#if isLeader(g) && invitingGroupId === g.groupId && latestInvites.length > 0}
                <div class="rounded-md bg-muted/50 p-3">
                  <div class="mb-2 text-xs font-medium text-muted-foreground">New Invite Codes:</div>
                  <div class="flex flex-wrap gap-2">
                    {#each latestInvites as code}<Badge
                        variant="outline"
                        class="cursor-text bg-background font-mono select-all">{code}</Badge
                      >{/each}
                  </div>
                </div>
              {/if}
              {#if Array.isArray(g.members) && g.members.length}<Separator />
                <div class="space-y-2">
                  <div class="text-sm font-medium">Members</div>
                  <div class="flex flex-wrap gap-2">
                    {#each g.members as m (m)}<Badge variant="secondary">{m}</Badge>{/each}
                  </div>
                </div>{/if}
            </div>
          {/each}
